...
def split_file(input_file_path, output_directory_path, split_size, equal_splits):
    ...
    if input_file_path.upper().endswith(VIDEO_SUFFIXES):
        ...
        for i in range(1, number_of_splits + 1):
            ...
            subprocess.run(["ffmpeg","-y", "-hide_banner", "-loglevel", "error", "-i",
                            input_file_path, "-ss", str(start_time), "-fs", str(split_size),
                            "-async", "1", "-strict", "-2", "-c", "copy", split_file_path])
            
            split_file_size = os.path.getsize(split_file_path)
            
            if split_file_size > 2097152000:
                dif = split_file_size - 2097152000
                split_size = split_size - dif + 104857600
                os.remove(split_file_path)
                split_file(input_file_path, output_directory_path, split_size, equal_splits=True)
        ...
        split_file_path = f"{input_file_path}."
        subprocess.run(["split", "--numeric-suffixes=1", "--suffix-length=3", f"--bytes={split_size}", input_file_path, split_file_path])
